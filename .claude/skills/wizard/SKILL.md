...
**ALWAYS** prefix your first response with `## [WIZARD MODE]` once, to signal architect-level standards are active. Do NOT repeat this as a heading at every phase transition — instead prepend a one-line inline tag `[P<n>/8 <Name>]` to whatever status line that phase already emits (see each phase's Checkpoint below). One header per run, one line per phase — not a banner per phase.

## Output Mode

Default output is the condensed one-line-per-phase format used throughout this doc (the `[P<n>/8 ...]` Checkpoint lines + the single closing Summary line). This is not optional prose — it is the floor.

**Quiet mode** — invoked via `/wizard --quiet` (or `-q`) in the command args, or the user saying "quiet mode" / "minimal output": suppress the closing Summary block in favor of one final result line (`done — <branch/PR or files changed>, <pass/fail>`), and suppress Phase 8.5 wakeup-sweep narration entirely (state-changes only, e.g. `PR #4 merge-ready`). Quiet mode is an additional suppression layer on top of the default — it never restores prose that the default already trims.

**Never suppressed, in either mode:** errors, blockers, adversarial findings needing a user decision, PR review findings, and the merge-ready declaration. These carry real informational stakes — they are status, not narration.
...
## Git Environment (auto-detected)

At the start of every wizard session, detect the environment once:

```bash
if git rev-parse --is-inside-work-tree > /dev/null 2>&1; then
  GIT_MODE=true   # full workflow
else
  GIT_MODE=false  # no commit / push / PR / worktree phases
fi
```

**When GIT_MODE=false:**
- Phase 7 Pre-Commit Review: omit the commit step; return a file-change summary instead of branch + SHA.
- Phase 8 PR & Review Cycle: **SKIP entirely.**
- Phase 8.5 Parallel Pipeline / Worktrees: **SKIP.** Manage parallel work via task isolation (separate subdirectories or sequential dispatch) instead.
- Threading model commit boundary: replaced by "worker completes file writes and returns changed-file list."
- Issue tracking: use a local `TASKS.md` or the task tool instead of a git-host issue tracker.
- All `git` commands in PATTERNS.md (fetch, diff, add, commit…): **no-op — skip them.**
- Agent return contract: return changed files list + completion summary instead of branch + SHA.

Everything else — TDD, phased planning, adversarial review, agent ensemble, complexity gate — works identically in both modes.
...
**Checkpoint:** `[P1/8 Plan] <one-clause scope + complexity band>` — ask clarifying questions only if genuinely blocked.
...
**Checkpoint:** `[P2/8 Explore] <N files to modify, pattern found>`.
...
**Checkpoint:** `[P3/8 TDD] RED confirmed failing → GREEN passing`.
...
**Checkpoint:** `[P4/8 Implement] done — all new tests passing`.
...
**Checkpoint:** `[P5/8 Verify] <suite> N passed / M failed` — if any failed, fix root cause and re-run before continuing.
...
**Checkpoint:** `[P6/8 Docs] docs/issue checkboxes current`.

---

## Phase 7: Pre-Commit Review

Run the **Pre-Commit Checklist** in [`CHECKLISTS.md`](CHECKLISTS.md) and the **Final adversarial questions** in [`CHECKLISTS.md`](CHECKLISTS.md#adversarial-questions) silently — verify each item against the diff, do not paste the checklist into chat. Surface only items that are **unmet**: `[P7/8 Review] <unmet item> — <fix taken>`, repeated per unmet item. If everything passes, one line: `[P7/8 Review] checklist clean`.
...
**Narrate state changes only, never the sweep mechanics.** Don't report "checked worktree A, checked worktree B, audited PR #3, no findings" — that's the algorithm running normally and carries no information. Emit a line only when a sweep step *changes* something: `PR #4 merge-ready`, `PR #2 finding fixed + pushed`, `worktree C crashed → re-dispatched`, `cohort refilled: spawned #7`. A wakeup with nothing to report emits nothing.
...
The roster lives in [`agents/`](../../agents/): `architect`, `backend-expert`, `frontend-expert`, `qa-engineer`, `doc-librarian`, `issue-maintainer`. Persona lenses for this project: `structural-engineer-lens`, `pinn-researcher-lens`, `headless-ci-runner-lens`, `performance-engineer-lens`. (`domain-user-lens` is the blank template — do not dispatch it directly.) `verbosity-budget-lens` is a process lens, not a persona lens — dispatch it during Phase 7 self-review and periodically during Phase 8.5 to guard against this skill's own output-verbosity rules regressing.

---

## Summary Output

After all phases, one combined block — only the lines with non-trivial content, no placeholder lines for items that didn't change:

```
[Done] <what was built, one clause>
files: <modified/added, comma-separated>
tests: <added/modified, only if non-empty>
docs/issue: <only if something changed>
PR: <link, checks state, findings-resolved state — only if GIT_MODE=true>
next: <only if there is a genuine next step>
```

In quiet mode, collapse this to the single result line described under **Output Mode**.
