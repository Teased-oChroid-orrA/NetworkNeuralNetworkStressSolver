...
This notification is the one piece of Phase 8.5 narration that is never trimmed, in quiet mode or
otherwise — it's the user's only signal that something needs them. Everything else in the
wakeup-sweep (steps 1–6 of the algorithm above) stays state-changes-only per SKILL.md's Phase 8.5
narration rule — quiet mode doesn't add a new rule here, it's already the floor.
